...
they have at home and have a shopping list to help them expedite their grocery shopping process.
...
You can increase the quantity of an ingredient that is already in the recipe, provided that you indicate a quantity with
...
Sample command feedback: `New ingredient added for Aglio olio: 2 cloves Garlic`

|`recipe 2 add ingredient i/Eggs q/2`
|Adds 2 eggs to the second recipe's list of ingredients.
Sample command feedback: `New ingredient added for Carbonara: 2 Eggs`
|===
...
Step 3: To add the first step the instant ramen recipe, type `recipe 3 add ingredient x/next s/Add the oil, garlic, and ginger to a small sauce pot and saute over medium for about one minute.` into the command box and press kbd:[Enter] on the keyboard.
...
|``search tag t/TAG``
|Search for recipes by a tag
|``search inventory``
|Search for recipes by the ingredients in the inventory.
|``list``
|List all recipes in the cookbook.
