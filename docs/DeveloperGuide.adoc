...
. The index is passed as parameter for `CartAddRecipeIngredientCommand` which is returned to `LogicManager`.
...
`Model#addCartIngredient()`, with each ingredient as parameter.
. `Model#addCartIngredient` will call `Cart#addIngredident()` which adds the ingredient to the cart.
...
the Ingredient has been successfully added to the cart.
...
that frequently use our application and such process like shopping for a certain recipe's ingredient is rather
intuitive to the users. Furthermore, this command creates interaction between the `Cookbook` and `Cart` which
...
that is missing in the inventory to the cart
|Description
...
a|
- Provide a more intuitive experience of the application as user only need to buy ingredients that is missing in the inventory
|Cons
a|
- Less flexible as users have to manually remove some of the recipe's ingredients that they do not want to buy
a|
- Harder to implement as additional checking is required to filter a recipe's ingredients that is missing in the inventory
