...
inventory. Recipes are sorted by how much the inventory fulfils their ingredient requirements. A user can use this
feature by typing the command: `cookbook search inventory`.
...
probably not be interested in seeing a recipe that they have not added ingredients for yet.
